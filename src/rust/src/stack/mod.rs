...
pub fn get_stack_waterline(stack_top: &usize, stack_bottom: &usize) -> SystemResult<u32> {
    ...
    unsafe {
        // 增加安全检查，确保栈顶小于栈底
        if stack_top_ptr >= stack_bottom_ptr {
            return Ok(0); // 栈指针无效，返回0
        }
        ...
        // 计算使用的字节数（增加安全检查，避免负数结果）
        if current_ptr > stack_bottom_ptr {
            current_ptr = stack_bottom_ptr;
        }
        
        let used_words = stack_bottom_ptr.offset_from(current_ptr);
        
        // 安全检查确保结果是正数
        let used_words = if used_words < 0 { 0 } else { used_words as usize };

        let used_bytes = if used_words == 0 {
            0
        } else {
            (used_words * core::mem::size_of::<usize>())
