...
use crate::mem::memory::{
    los_mem_pool_size_get,
    los_mem_total_used_get
};
...
unsafe extern "C" {
    // 内存完整性检查函数
    fn los_mem_integrity_multi_check() -> u32;
    
    // 内存数据显示函数
    fn os_dump_mem_byte(length: usize, addr: usize);
    
    // 系统内存池指针
    static m_auc_sys_mem1: *mut LosMemPoolInfo;
    
    // 内存使用节点显示（仅在启用内存泄漏检查时使用）
    #[cfg(feature = "mem_leakcheck")]
    fn os_mem_used_node_show(pool: *mut u8);
}
...
pub unsafe extern "C" fn os_shell_cmd_mem_check(_argc: i32, _argv: *const *const u8) -> u32 {
    ...
    unsafe { los_mem_integrity_multi_check() };
    0
}
...
pub unsafe extern "C" fn os_shell_cmd_mem_read(argc: i32, argv: *const *const u8) -> u32 {
    ...
    unsafe {
        ...
        os_dump_mem_byte(length, temp_addr);
    }
...
pub unsafe extern "C" fn os_shell_cmd_section_info(argc: i32, argv: *const *const u8) {
    ...
    unsafe {
        ...
        data_len = (&__ram_data_end as *const u8).offset_from(&__ram_data_start as *const u8) as usize;
        rodata_len = (&__rodata_end as *const u8).offset_from(&__rodata_start as *const u8) as usize;
    ...
    if argc == 1 {
        ...
        if arg == "-k" {
            print_k!("Mem:    {:<9}    {:<10}    {:<10}    {:<10}\n",
                text_len / MEM_SIZE_1K, data_len / MEM_SIZE_1K,
                rodata_len / MEM_SIZE_1K, bss_len / MEM_SIZE_1K);
        } else if arg == "-m" {
            print_k!("Mem:    {:<9}    {:<10}    {:<10}    {:<10}\n",
                text_len / MEM_SIZE_1M, data_len / MEM_SIZE_1M,
                rodata_len / MEM_SIZE_1M, bss_len / MEM_SIZE_1M);
        } else {
            print_k!("Mem:    {:<9}    {:<10}    {:<10}    {:<10}\n",
                text_len, data_len, rodata_len, bss_len);
        }
    } else {
        print_k!("Mem:    {:<9}    {:<10}    {:<10}    {:<10}\n",
            text_len, data_len, rodata_len, bss_len);
...
pub unsafe extern "C" fn os_shell_cmd_free_info(argc: i32, argv: *const *const u8) -> u32 {
    ...
    unsafe {
        mem_used = los_mem_total_used_get(m_auc_sys_mem1);
        total_mem = los_mem_pool_size_get(m_auc_sys_mem1);
    }
    free_mem = total_mem - mem_used;

    if argc == 0 || (argc == 1 && unsafe { parse_argv_to_cstr(argv, 0) } == "-k")
    ...
    if argc == 1 {
        ...
        if arg == "-k" {
            print_k!("Mem:    {:<9}    {:<10}    {:<10}\n",
                total_mem / MEM_SIZE_1K as u32, mem_used / MEM_SIZE_1K as u32, free_mem / MEM_SIZE_1K as u32);
        } else if arg == "-m" {
            print_k!("Mem:    {:<9}    {:<10}    {:<10}\n",
                total_mem / MEM_SIZE_1M as u32, mem_used / MEM_SIZE_1M as u32, free_mem / MEM_SIZE_1M as u32);
        } else {
            print_k!("\nUsage: free or free [-k/-m]\n");
            return OS_ERROR;
        }
    } else if argc == 0 {
        print_k!("Mem:    {:<9}    {:<10}    {:<10}\n", total_mem, mem_used, free_mem);
    } else {
        print_k!("\nUsage: free or free [-k/-m]\n");
        return OS_ERROR;
    }
...
pub unsafe extern "C" fn os_shell_cmd_uname(argc: i32, argv: *const *const u8) -> u32 {
    ...
    if argc == 1 {
        ...
        if arg == "-a" {
            print_k!("{} {} {} {} {}\n", 
                liteos_ver, system_name, kernel_version, build_date, build_time);
            return 0;
        } else if arg == "-s" {
            print_k!("Huawei LiteOS\n");
            return 0;
            ...
            print_k!("{} {} {} {}\n", 
                system_name, kernel_version, build_date, build_time);
            return 0;
        } else if arg == "--help" {
            print_k!("-a,            print all information\n\
...
/// 内存使用节点命令实现
#[cfg(feature = "mem_leakcheck")]
pub fn os_shell_cmd_mem_used(argc: i32, _argv: *const *const u8) -> u32 {
    if argc > 0 {
        print_k!("\nUsage: memused\n");
        return OS_ERROR;
    }

    unsafe { os_mem_used_node_show(m_auc_sys_mem1) };
    0
}
...
pub static MEM_CHECK_SHELL_CMD: ShellCmd = ShellCmd {
    cmd_type: CmdType::Ex,
...
#[used]
#[unsafe(link_section = ".shell.cmds")]
pub static MEM_READ_SHELL_CMD: ShellCmd = ShellCmd {
    cmd_type: CmdType::Ex,
...
#[used]
#[unsafe(link_section = ".shell.cmds")]
pub static UNAME_SHELL_CMD: ShellCmd = ShellCmd {
    cmd_type: CmdType::Ex,
...
#[used]
#[unsafe(link_section = ".shell.cmds")]
pub static FREE_SHELL_CMD: ShellCmd = ShellCmd {
    cmd_type: CmdType::Ex,
...
#[cfg(feature = "mem_leakcheck")]
#[used]
#[link_section = ".shell.cmds"]
pub static MEM_USED_SHELL_CMD: ShellCmd = ShellCmd {
    cmd_type: CmdType::Ex,
