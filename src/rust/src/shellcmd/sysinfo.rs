...
use crate::semaphore::types::{SemaphoreControlBlock as LosSemCB, SemaphoreState};

#[cfg(feature = "base_ipc_mux")]
use crate::mutex::types::{MutexControlBlock as LosMuxCB, MutexState};

#[cfg(feature = "base_ipc_queue")]
use crate::queue::types::{QueueControlBlock as LosQueueCB, QueueState};
...
unsafe fn get_sem(sem_id: u32) -> *mut LosSemCB {
    &mut g_allSem[get_sem_index(sem_id) as usize]
}
...
unsafe extern "C" {
    ...
    #[cfg(feature = "base_ipc_sem")]
    #[link_name = "g_allSem"] // 显式指定链接名称
    static mut g_allSem: [LosSemCB; 1024];
