...
#define NODEDUMPSIZE 64
/* column num of the output info of mem node */
#define COLUMN_NUM 8

#define OS_CHECK_NULL_RETURN(param)                                            \
  do {                                                                         \
    if ((param) == NULL) {                                                     \
...
STATIC INLINE VOID OsMemSetMagicNumAndTaskID(LosMemDynNode *node) {
  ...
  /*
   * If the operation occurred before task initialization(runTask was not
   * assigned) or in interrupt, make the value of taskid of node to  (TASK_NUM -
   * 1) which is the id of the extra task. We can record those memory use with
   * that.
   */
...
#define OS_MEM_NODE_CHECKSUM_CALCULATE(ctlNode)                                \
  (((UINTPTR)(ctlNode)->freeNodeInfo.pstPrev) ^                                \
   ...
   ((UINTPTR)(ctlNode)->preNode) ^ (ctlNode)->gapSize ^                        \
   (ctlNode)->sizeAndFlag ^ CHECKSUM_MAGICNUM)
...
STATIC INLINE VOID OsMemDispCtlNode(const LosMemCtlNode *ctlNode) {
  ...
  PRINTK("node:%p checksum=%p[%p] freeNodeInfo.pstPrev=%p "
         ...
         ctlNode->freeNodeInfo.pstNext, ctlNode->preNode, ctlNode->gapSize,
         ctlNode->sizeAndFlag);
}
...
STATIC INLINE VOID OsMemDispWildPointerMsg(const LosMemDynNode *node,
                                           const VOID *ptr) {
  PRINT_ERR("*****************************************************\n");
  PRINT_ERR("find an control block at: %p, gap size: 0x%x, "
            "sizeof(LosMemDynNode): 0x%x\n",
            node, node->selfNode.gapSize, sizeof(LosMemDynNode));
...
STATIC INLINE VOID OsMemBackupSetup(const LosMemDynNode *node) {
  ...
  if (nodePre != NULL) {
    nodePre->backupNode.freeNodeInfo.pstNext =
        node->selfNode.freeNodeInfo.pstNext;
    nodePre->backupNode.freeNodeInfo.pstPrev =
        node->selfNode.freeNodeInfo.pstPrev;
...
STATIC INLINE UINT32 OsMemBackupSetup4Next(const VOID *pool,
                                           LosMemDynNode *node) {
  ...
  if (!OsMemChecksumVerify(&node->backupNode)) {
    node->backupNode.freeNodeInfo.pstNext =
        nodeNext->selfNode.freeNodeInfo.pstNext;
    node->backupNode.freeNodeInfo.pstPrev =
...
UINT32 OsMemBackupDoRestore(VOID *pool, const LosMemDynNode *nodePre,
                            LosMemDynNode *node) {
  if (node == NULL) {
...
STATIC LosMemDynNode *OsMemFirstNodePrevGet(const LosMemPoolInfo *poolInfo) {
  ...
  if (!OsMemChecksumVerify(&(nodePre->backupNode))) {
    PRINT_ERR("the current node is THE FIRST NODE !\n");
    PRINT_ERR("[%s]: the backup node information of current node in previous "
              "Node is bad !!\n",
              __FUNCTION__);
    OsMemDispCtlNode(&(nodePre->backupNode));
    return NULL;
...
LosMemDynNode *OsMemNodePrevTryGet(VOID *pool, LosMemDynNode **node,
                                   const VOID *ptr) {
  ...
  for (nodeCur = OS_MEM_FIRST_NODE(pool);
    ...
    nodeShouldBe =
        (UINTPTR)nodeCur + nodeCur->selfNode.gapSize + sizeof(LosMemDynNode);
...
STATIC INLINE UINT32 OsMemBackupTryRestore(VOID *pool, LosMemDynNode **node,
                                           const VOID *ptr) {
...
STATIC INLINE UINT32 OsMemBackupCheckAndRetore(VOID *pool, const VOID *ptr,
                                               LosMemDynNode *node) {
  LosMemPoolInfo *poolInfo = (LosMemPoolInfo *)pool;
...
STATIC VOID OsMemListDelete(LOS_DL_LIST *node, const VOID *firstNode) {
  ...
  if ((VOID *)(node->pstNext) >= firstNode) {
    dynNode =
        LOS_DL_LIST_ENTRY(node->pstNext, LosMemDynNode, selfNode.freeNodeInfo);
    OsMemNodeSave(dynNode);
  }

  if ((VOID *)(node->pstPrev) >= firstNode) {
    dynNode =
        LOS_DL_LIST_ENTRY(node->pstPrev, LosMemDynNode, selfNode.freeNodeInfo);
    OsMemNodeSave(dynNode);
  }
...
STATIC VOID OsMemListAdd(LOS_DL_LIST *listNode, LOS_DL_LIST *node,
  ...
  if ((VOID *)(listNode->pstNext) >= firstNode) {
    dynNode = LOS_DL_LIST_ENTRY(listNode->pstNext, LosMemDynNode,
...
VOID LOS_MemBadNodeShow(VOID *pool) {
  ...
  for (tmpNode = OS_MEM_FIRST_NODE(pool);
       tmpNode <= OS_MEM_END_NODE(pool, poolInfo->poolSize);
...
OsMemLinkRegisterRecord(LosMemDynNode *node) {
  UINTPTR framePtr;

  (VOID) memset_s(node->selfNode.linkReg, (LOS_RECORD_LR_CNT * sizeof(UINTPTR)),
                  0, (LOS_RECORD_LR_CNT * sizeof(UINTPTR)));
  framePtr = ArchGetFp();
  (VOID) ArchBackTraceGet(framePtr, node->selfNode.linkReg, LOS_RECORD_LR_CNT);
  return;
}
...
STATIC VOID *OsMemReallocSlab(VOID *pool, VOID *ptr, BOOL *isSlabMem,
                              UINT32 size) {
  errno_t rc;
  UINT32 blkSz;
  VOID *newPtr = NULL;
...
STATIC VOID *OsMemReallocSlab(VOID *pool, const VOID *ptr, BOOL *isSlabMem,
                              UINT32 size) {
  *isSlabMem = FALSE;
  return NULL;
}
 ...
 */
STATIC INLINE LosMemDynNode *OsMemFindSuitableFreeBlock(VOID *pool,
                                                        UINT32 allocSize) {
  ...
  for (listNodeHead = OS_MEM_HEAD(pool, allocSize); listNodeHead != NULL;
       listNodeHead =
           OsDLnkNextMultiHead(OS_MEM_HEAD_ADDR(pool), listNodeHead)) {
    LOS_DL_LIST_FOR_EACH_ENTRY(tmpNode, listNodeHead, LosMemDynNode,
      ...
      if (((UINTPTR)tmpNode < (UINTPTR)pool) ||
          ((UINTPTR)tmpNode >
           ((UINTPTR)pool + ((LosMemPoolInfo *)pool)->poolSize)) ||
          (((UINTPTR)tmpNode & (OS_MEM_ALIGN_SIZE - 1)) != 0)) {
        LOS_Panic("[%s:%d]Mem node data error:OS_MEM_HEAD_ADDR(pool)=%p, "
                  "listNodeHead:%p,"
                  "allocSize=%u, tmpNode=%p\n",
                  __FUNCTION__, __LINE__, OS_MEM_HEAD_ADDR(pool), listNodeHead,
                  allocSize, tmpNode);
        break;
      }
#endif
...
STATIC INLINE VOID OsMemClearNode(LosMemDynNode *node) {
  (VOID)
      memset_s((VOID *)node, sizeof(LosMemDynNode), 0, sizeof(LosMemDynNode));
}
...
/*
 * Description : split new node from allocNode, and merge remainder mem if
 * necessary Input       : pool      -- Pointer to memory pool allocNode -- the
 * source node which the new node will split from it. After pick up it's node
 * info, change to point to the new node allocSize -- the size of new node
 * Output      : allocNode -- save new node addr
 */
STATIC INLINE VOID OsMemSplitNode(VOID *pool, LosMemDynNode *allocNode,
                                  UINT32 allocSize) {
  LosMemDynNode *newFreeNode = NULL;
  LosMemDynNode *nextNode = NULL;
  LOS_DL_LIST *listNodeHead = NULL;
  const VOID *firstNode =
      (const VOID *)((UINT8 *)OS_MEM_HEAD_ADDR(pool) + OS_DLNK_HEAD_SIZE);

  newFreeNode = (LosMemDynNode *)(VOID *)((UINT8 *)allocNode + allocSize);
  newFreeNode->selfNode.preNode = allocNode;
  newFreeNode->selfNode.sizeAndFlag =
...
/*
 * Description : free the node from memory & if there are free node beside,
 * merger them. at last update "listNodeHead' which saved all free node control
 * head Input       : node -- the node which need be freed pool -- Pointer to
 * memory pool
 */
STATIC INLINE VOID OsMemFreeNode(LosMemDynNode *node, LosMemPoolInfo *pool) {
  ...
  OS_MEM_REDUCE_USED(&pool->stat,
                     OS_MEM_NODE_GET_SIZE(node->selfNode.sizeAndFlag),
  ...
  if ((node->selfNode.preNode != NULL) &&
      !OS_MEM_NODE_GET_USED_FLAG(
          node->selfNode.preNode->selfNode.sizeAndFlag)) {
...
#ifdef LOSCFG_MEM_DEBUG
/*
 * Description : check the result if pointer memory node belongs to pointer
 * memory pool Input       : pool -- Pointer to memory pool node -- the node
 * which need be checked Return      : LOS_OK or LOS_NOK
 */
STATIC INLINE BOOL OsMemIsNodeValid(const LosMemDynNode *node,
                                    const LosMemDynNode *startNode,
                                    const LosMemDynNode *endNode,
                                    const UINT8 *startPool,
                                    const UINT8 *endPool) {
  if (!OS_MEM_MIDDLE_ADDR(startNode, node, endNode)) {
    return FALSE;
  }
  ...
  if (!OS_MEM_MIDDLE_ADDR_OPEN_END(
          startPool, node->selfNode.freeNodeInfo.pstPrev, endPool)) {
    return FALSE;
  }
...
STATIC INLINE VOID OsMemCheckUsedNode(const VOID *pool,
                                      const LosMemDynNode *node) {
  const LosMemPoolInfo *poolInfo = (const LosMemPoolInfo *)pool;
  const LosMemDynNode *startNode =
      (const LosMemDynNode *)OS_MEM_FIRST_NODE(pool);
  const LosMemDynNode *endNode =
      (const LosMemDynNode *)OS_MEM_END_NODE(pool, poolInfo->poolSize);
  ...
  if ((!OsMemIsNodeValid(node, startNode, endNode, (UINT8 *)pool, endPool)) ||
      (!OS_MEM_NODE_GET_USED_FLAG(node->selfNode.sizeAndFlag))) {
    LOS_Panic("The node:%x has been damaged!\n", node);
  }

  nextNode = OS_MEM_NEXT_NODE(node);
  if ((!OsMemIsNodeValid(nextNode, startNode, endNode, (UINT8 *)pool,
                         endPool)) ||
  ...
  if (node != startNode) {
    if ((!OsMemIsNodeValid(node->selfNode.preNode, startNode, endNode,
                           (UINT8 *)pool, endPool)) ||
...
#ifdef LOSCFG_BASE_MEM_NODE_INTEGRITY_CHECK
LITE_OS_SEC_TEXT_MINOR STATIC INLINE UINT32
OsMemPoolDlinkcheck(const LosMemPoolInfo *pool, LOS_DL_LIST listHead) {
  ...
  for (dlinkNum = 0; dlinkNum < OS_MULTI_DLNK_NUM; dlinkNum++) {
    if (OsMemPoolDlinkcheck(pool, dlinkHead->listHead[dlinkNum])) {
      ...
      PRINT_ERR("DlinkHead[%u]: pstPrev:%p, pstNext:%p\n", dlinkNum,
                dlinkHead->listHead[dlinkNum].pstPrev,
                dlinkHead->listHead[dlinkNum].pstNext);
#ifdef LOSCFG_SHELL_EXCINFO_DUMP
...
STATIC UINT32 OsMemIntegrityCheck(const VOID *pool, LosMemDynNode **tmpNode,
    ...
    if (OS_MEM_NODE_GET_USED_FLAG((*tmpNode)->selfNode.sizeAndFlag)) {
      if (!OS_MEM_MAGIC_VALID((*tmpNode)->selfNode.magic)) {
        PRINT_ERR("[%s], %d, memory check error!\n"
                  "memory used but magic num wrong, freeNodeInfo.pstPrev(magic "
                  "num):%p \n",
                  __FUNCTION__, __LINE__,
                  (*tmpNode)->selfNode.freeNodeInfo.pstPrev);
#ifdef LOSCFG_SHELL_EXCINFO_DUMP
        WriteExcInfoToBuf("[%s], %d, memory check error!\n"
                          "memory used but magic num wrong, "
                          "freeNodeInfo.pstPrev(magic num):%p \n",
                          __FUNCTION__, __LINE__,
                          (*tmpNode)->selfNode.freeNodeInfo.pstPrev);
#endif
        return LOS_NOK;
      ...
      if (!OS_MEM_MIDDLE_ADDR_OPEN_END(
              pool, (*tmpNode)->selfNode.freeNodeInfo.pstPrev, endPool)) {
        PRINT_ERR(
      ...
      if (!OS_MEM_MIDDLE_ADDR_OPEN_END(
              pool, (*tmpNode)->selfNode.freeNodeInfo.pstNext, endPool)) {
        PRINT_ERR(
            "[%s], %d, memory check error!\n"
            " freeNodeInfo.pstNext:%p is out of legal mem range[%p, %p]\n",
            __FUNCTION__, __LINE__, (*tmpNode)->selfNode.freeNodeInfo.pstNext,
            pool, endPool);
#ifdef LOSCFG_SHELL_EXCINFO_DUMP
        WriteExcInfoToBuf(
            "[%s], %d, memory check error!\n"
...
STATIC VOID OsMemNodeInfo(const LosMemDynNode *tmpNode,
  ...
  PRINTK("\n broken node head: %p  %p  %p  0x%x, pre node head: %p  %p  %p  "
         "0x%x\n",
         tmpNode->selfNode.freeNodeInfo.pstPrev,
         tmpNode->selfNode.freeNodeInfo.pstNext, tmpNode->selfNode.preNode,
         tmpNode->selfNode.sizeAndFlag, preNode->selfNode.freeNodeInfo.pstPrev,
         preNode->selfNode.freeNodeInfo.pstNext, preNode->selfNode.preNode,
         preNode->selfNode.sizeAndFlag);

#ifdef LOSCFG_SHELL_EXCINFO_DUMP
  WriteExcInfoToBuf("\n broken node head: %p  %p  %p  0x%x, pre node head: %p  "
                    "%p  %p  0x%x\n",
                    tmpNode->selfNode.freeNodeInfo.pstPrev,
                    tmpNode->selfNode.freeNodeInfo.pstNext,
                    tmpNode->selfNode.preNode, tmpNode->selfNode.sizeAndFlag,
                    preNode->selfNode.freeNodeInfo.pstPrev,
                    preNode->selfNode.freeNodeInfo.pstNext,
...
STATIC INLINE VOID OsMemNodeDebugOperate(VOID *pool, LosMemDynNode *allocNode,
                                         UINT32 size) {
#ifdef LOSCFG_MEM_HEAD_BACKUP
...
STATIC UINT32 OsMemInfoGet(const VOID *pool, LOS_MEM_POOL_STATUS *poolStatus) {
  ...
  for (tmpNode = OS_MEM_FIRST_NODE(poolInfo);
       tmpNode <= OS_MEM_END_NODE(poolInfo, poolInfo->poolSize);
       tmpNode = OS_MEM_NEXT_NODE(tmpNode)) {
    if (!OS_MEM_NODE_GET_USED_FLAG(tmpNode->selfNode.sizeAndFlag)) {
      ++freeNodeNum;
      totalFreeSize += OS_MEM_NODE_GET_SIZE(tmpNode->selfNode.sizeAndFlag);
      if (maxFreeNodeSize <
          OS_MEM_NODE_GET_SIZE(tmpNode->selfNode.sizeAndFlag)) {
...
STATIC VOID *OsMemAllocWithCheck(LosMemPoolInfo *pool, UINT32 size) {
  ...
  if ((allocSize + OS_MEM_NODE_HEAD_SIZE + OS_MEM_ALIGN_SIZE) <=
      allocNode->selfNode.sizeAndFlag) {
  ...
  OS_MEM_ADD_USED(&pool->stat,
                  OS_MEM_NODE_GET_SIZE(allocNode->selfNode.sizeAndFlag),
                  OS_MEM_TASKID_GET(allocNode));
  OsMemNodeDebugOperate(pool, allocNode, size);

#ifdef LOSCFG_KERNEL_LMS
  if (g_lmsMallocHook != NULL) {
    g_lmsMallocHook(allocNode + 1);
  }
#endif
  return (allocNode + 1);
}
 ...
 */
STATIC INLINE VOID OsMemReAllocSmaller(LosMemPoolInfo *pool, UINT32 allocSize,
                                       LosMemDynNode *node, UINT32 nodeSize) {
  if ((allocSize + OS_MEM_NODE_HEAD_SIZE + OS_MEM_ALIGN_SIZE) <= nodeSize) {
    ...
    OS_MEM_REDUCE_USED(&pool->stat, nodeSize - allocSize,
                       OS_MEM_TASKID_GET(node));
  }
#ifdef LOSCFG_MEM_LEAKCHECK
  OsMemLinkRegisterRecord(node);
 ...
 * Output      : node      --- pointer to the new node after realloc
 */
STATIC INLINE VOID OsMemMergeNodeForReAllocBigger(LosMemPoolInfo *pool,
                                                  UINT32 allocSize,
                                                  LosMemDynNode *node,
                                                  UINT32 nodeSize,
                                                  LosMemDynNode *nextNode) {
  const VOID *firstNode =
      (const VOID *)((UINT8 *)OS_MEM_HEAD_ADDR(pool) + OS_DLNK_HEAD_SIZE);
  ...
  OsMemMergeNode(nextNode);
  if ((allocSize + OS_MEM_NODE_HEAD_SIZE + OS_MEM_ALIGN_SIZE) <=
      node->selfNode.sizeAndFlag) {
    OsMemSplitNode(pool, node, allocSize);
  }

  OS_MEM_ADD_USED(&pool->stat, node->selfNode.sizeAndFlag - nodeSize,
                  OS_MEM_TASKID_GET(node));
...
STATIC UINT32 OsMemInit(VOID *pool, UINT32 size) {
...
#ifdef LOSCFG_KERNEL_LMS
  if (g_lmsMemInitHook != NULL) {
    poolSize = g_lmsMemInitHook(pool, size);
    if (poolSize == 0) {
      poolSize = size;
    }
  }
  ...
  newNode->selfNode.sizeAndFlag =
      (poolSize - (UINT32)((UINTPTR)newNode - (UINTPTR)pool) -
  ...
  poolInfo->stat.memTotalUsed =
      sizeof(LosMemPoolInfo) + OS_MULTI_DLNK_HEAD_SIZE +
...
LITE_OS_SEC_TEXT_INIT UINT32 LOS_MemInit(VOID *pool, UINT32 size) {
  ...
  if (!IS_ALIGNED(size, OS_MEM_ALIGN_SIZE) ||
      !IS_ALIGNED(pool, OS_MEM_ALIGN_SIZE)) {
    PRINT_WARN(
        "pool [%p, %p) size 0x%x should be aligned with OS_MEM_ALIGN_SIZE\n",
...
LITE_OS_SEC_TEXT VOID *LOS_MemAllocAlign(VOID *pool, UINT32 size,
                                         UINT32 boundary) {
  UINT32 useSize;
  UINT32 gapSize;
  VOID *ptr = NULL;
  ...
  if ((pool == NULL) || (size == 0) || (boundary == 0) ||
      !IS_POW_TWO(boundary) || !IS_ALIGNED(boundary, sizeof(VOID *))) {
    return NULL;
  }
  ...
  do {
    ...
    if (OS_MEM_NODE_GET_USED_FLAG(useSize) ||
        OS_MEM_NODE_GET_ALIGNED_FLAG(useSize)) {
      break;
    }
...
LITE_OS_SEC_TEXT STATIC INLINE VOID OsDoMemFree(VOID *pool, const VOID *ptr,
                                                LosMemDynNode *node) {
  OsMemCheckUsedNode(pool, node);

  OsMemFreeNode(node, pool);
#ifdef LOSCFG_KERNEL_LMS
  if (g_lmsFreeHook != NULL) {
    g_lmsFreeHook(ptr);
  }
#endif
}
...
UINT32 OsMemFree(VOID *pool, const VOID *ptr) {
  ...
  do {
    ...
    if (OS_MEM_NODE_GET_ALIGNED_FLAG(gapSize) &&
        OS_MEM_NODE_GET_USED_FLAG(gapSize)) {
...
LITE_OS_SEC_TEXT UINT32 LOS_MemFree(VOID *pool, VOID *ptr) {
  ...
  if ((pool == NULL) || (ptr == NULL) || !IS_ALIGNED(pool, sizeof(VOID *)) ||
      !IS_ALIGNED(ptr, sizeof(VOID *))) {
    return LOS_NOK;
  }
...
STATIC VOID *OsMemRealloc(VOID *pool, VOID *ptr, UINT32 size) {
  ...
  UINT32 nodeSize;
  UINT32 allocSize =
      OS_MEM_ALIGN(size + OS_MEM_NODE_HEAD_SIZE, OS_MEM_ALIGN_SIZE);
  ...
  if (tmpPtr != NULL) {
    ...
    if (memcpy_s(tmpPtr, size, ptr,
                 (nodeSize - OS_MEM_NODE_HEAD_SIZE - gapSize)) != EOK) {
...
LITE_OS_SEC_TEXT_MINOR VOID *LOS_MemRealloc(VOID *pool, VOID *ptr,
                                            UINT32 size) {
  UINT32 intSave;
  VOID *newPtr = NULL;
  ...
  for (tmpNode = OS_MEM_FIRST_NODE(poolInfo);
    ...
    if ((UINTPTR)ptr < (UINTPTR)tmpNode) {
      if (OS_MEM_NODE_GET_USED_FLAG(
              tmpNode->selfNode.preNode->selfNode.sizeAndFlag)) {
        MEM_UNLOCK(intSave);
...
LITE_OS_SEC_TEXT_MINOR VOID OsMemResetEndNode(VOID *pool, UINTPTR preAddr) {
  LosMemDynNode *endNode = (LosMemDynNode *)OS_MEM_END_NODE(
      pool, ((LosMemPoolInfo *)pool)->poolSize);
  endNode->selfNode.sizeAndFlag = OS_MEM_NODE_HEAD_SIZE;
  if (preAddr != 0) {
    endNode->selfNode.preNode =
        (LosMemDynNode *)(preAddr - sizeof(LosMemDynNode));
  }
  OS_MEM_NODE_SET_USED_FLAG(endNode->selfNode.sizeAndFlag);
  OsMemSetMagicNumAndTaskID(endNode);
...
LITE_OS_SEC_TEXT_MINOR UINT32 LOS_MemInfoGet(VOID *pool,
...
STATIC INLINE VOID OsShowFreeNode(UINT32 index, UINT32 length,
                                  const UINT32 *countNum) {
  UINT32 count = 0;
  PRINTK("\n    block size:  ");
  while (count < length) {
...
LITE_OS_SEC_TEXT_MINOR UINT32 LOS_MemFreeNodeShow(VOID *pool) {
  LOS_DL_LIST *listNodeHead = NULL;
  LosMultipleDlinkHead *headAddr =
      (LosMultipleDlinkHead *)((UINTPTR)pool + sizeof(LosMemPoolInfo));
  LosMemPoolInfo *poolInfo = (LosMemPoolInfo *)pool;
  ...
  while (linkHeadIndex < OS_MULTI_DLNK_NUM) {
    if (linkHeadIndex + COLUMN_NUM < OS_MULTI_DLNK_NUM) {
      ...
      OsShowFreeNode(linkHeadIndex, (OS_MULTI_DLNK_NUM - 1 - linkHeadIndex),
                     countNum);
      break;
    }
  }
...
LITE_OS_SEC_TEXT_MINOR UINT32 LOS_MemNodeSizeCheck(VOID *pool, VOID *ptr,
                                                   UINT32 *totalSize,
                                                   UINT32 *availSize) {
  ...
  if ((pool == NULL) || (ptr == NULL) || (totalSize == NULL) ||
      (availSize == NULL)) {
  ...
  if (g_memCheckLevel == LOS_MEM_CHECK_LEVEL_HIGH) {
    ...
    if ((head == NULL) ||
        (OS_MEM_NODE_GET_SIZE(((LosMemDynNode *)head)->selfNode.sizeAndFlag) <
         ((UINTPTR)ptr - (UINTPTR)head))) {
      return LOS_ERRNO_MEMCHECK_NO_HEAD;
    }
    *totalSize = OS_MEM_NODE_GET_SIZE(
        ((LosMemDynNode *)head)->selfNode.sizeAndFlag - sizeof(LosMemDynNode));
    *availSize =
        OS_MEM_NODE_GET_SIZE(((LosMemDynNode *)head)->selfNode.sizeAndFlag -
                             ((UINTPTR)ptr - (UINTPTR)head));
    return LOS_OK;
 ...
 * attention : this func couldn't ensure the return memCtrl belongs to ptr it
 * just find forward the most nearly one
 */
LITE_OS_SEC_TEXT_MINOR const VOID *OsMemFindNodeCtrl(const VOID *pool,
                                                     const VOID *ptr) {
  const VOID *head = ptr;
...
UINT32 OsMemSysNodeCheck(VOID *dstAddr, VOID *srcAddr, UINT32 nodeLength,
                         UINT8 pos) {
  UINT32 ret;
  UINT32 totalSize = 0;
  UINT32 availSize = 0;
  ...
  if ((ret == LOS_OK) && (nodeLength > availSize)) {
    PRINT_ERR("---------------------------------------------\n"
              "%s: dst inode availSize is not enough availSize = 0x%x, memcpy "
              "length = 0x%x\n",
...
VOID *LOS_MemMallocAlign(VOID *pool, UINT32 size, UINT32 boundary,
                         UINT32 moduleId) {
  UINT32 intSave;
  VOID *ptr = NULL;
  VOID *node = NULL;
